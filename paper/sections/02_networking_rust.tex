...
The module \rust{std::net} of the Rust standard library already implements this protocol, so you can work with these
...
\textcolor{teal}{Figure \ref{tcp-client-handeling}} demonstrates how messages sent to a \rust{std::net::TcpStream} can
...
and we can break out of the loop. If at least one byte is read, the received line is written into \rust{buf} and the
...
printed to the error output and the program tries to shutdown the stream using \rust{stream.shutdown(Both)?}. This
...
The example in \textcolor{teal}{Figure \ref{tcp-client-handeling}} can be used to handle a connection to a single host
...
\textcolor{teal}{Figure \ref{tcp-client-handeling}} is now wrapped into a function called \rust{handle_client}, taking
...
an argument. As the handling of the client can also result in errors, the \rust{expect} method is used once again to
...
First a connection to the server is established using the function \rust{TcpStream::connect}, which takes a \rust{&str}
\rust{ADDRESS} as an argument. The constant \rust{ADDRESS} is similar to the constant we have used in the server and
corresponds to the address and port of the server we want to connect to. This time no \rust{TcpListener} is needed,
because the other host is already listening for connections and we want to establish a connection explicitly to this
host. The returned result of the function is unwrapped using \rust{?} to return early with an error if something went
wrong.

In order to let the user send messages from the command line, we have to use the struct \rust{std::io::Stdin} which you
can get by calling the function \rust{std::io::stdin}. To prevent repeated calls of this function, it is called once in
the beginning and saved in a immutable variable.

After that the program enters a loop and keeps iterating until the connection was closed. In \textcolor{orange}{line 6}
a mutable \rust{String} called \rust{buf} is created. The variable is used to save the console input of the user and
...
"exit". In such case, the \rust{shutdown} method of the stream is used to close the current connection and the program
breaks out of the loop. Otherwise the input string is converted into a byte slice using the \rust{String}-method
\rust{as_bytes}. The method \rust{write_all} of the stream takes this slice as an argument and sends it directly to the
server. As this is another operation which might fail due to network issues, it again returns a \rust{Result} which is
unwrapped through the \rust{expect} method.
...
When trying to communicate over UDP in Rust, one is able to use the struct \rust{std::net::UdpSocket}. It provides
...
In the beginning a socket is created by calling the \rust{bind} function and giving the address the socket should listen
...
similar to interfaces in other languages. This means you cannot create a \rust{BufReader} instance in order to read the
data from the socket. Instead, we have to read the messages as bytes and then parse them into a \rust{String} later.
The variable \rust{buf} in this case is a byte slice of length \rust{1500}. If you know the size of the data
...
sender needs its own address and port in order to receive answers from the other host. By giving \rust{bind} the static
...
The result of the method call is again unwrapped using \rust{expect} to panic in case of an error.
