...
them into a String later. The variable \rust{buf} in this case is a byte slice of the length \rust{4096}. If you know
the size of the data beforehand, you can adjust the buffer length according to this, but as we do know anything about
the data we are going to receive, this example uses the size of the smallest page size in most computer architectures.
If a message is larger than the buffer, it is going to get cut off.
...
To send data to this receiver, you cannot just use the TCP client implemented before as it uses a differnt protocol. In
...
\rust{as_bytes} and then send to the other host by calling \rust{send_to} on the socket, giving it the byte slice and
