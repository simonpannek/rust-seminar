...
waiting for another task to finish before it can continue running, you can call \rust{await} on the method. This
...
The main function is marked as \rust{async} and the Tokio macro is used to create a runtime for our program to run in.
In \textcolor{orange}{line 10}, \rust{async_function} is called. The value returned by the function has the type
\rust{std::future::Future}, which wraps the value going to be returned after the computation. The \rust{Future} is
saved in the variable \rust{future1}. This way the Tokio runtime actually decides when to evaluate the function. If
\rust{async_function} was not marked using \rust{async}, the main thread executing the program would enter the
...
In an asynchronous environment on the other hand, the scheduler stops the execution of the current task and swaps in
another task which is waiting to be executed when the program reaches an \rust{await} point. This way, the sleep
function does not block the entire program flow and the function wrapped by \rust{future2} finishes first, even if only
...
Tokio offers the module \rust{tokio::net}, which contains a TCP and UDP network API similar to the one of the standard
library used in chapter \ref{networking-in-rust}. These functions are marked as \rust{async}, blending in with
...
As an argument the function is given an asynchronous code block, which returns a \rust{Future} as seen in Figure
\textcolor{teal}{\ref{future}}. Here additionaly to the \rust{async} keyword, \rust{move} is used. This means the
...
full ownership of the \rust{stream} variable in order to read from it and shut it down in case of an error.
...
created for more efficient reading from the \rust{TcpStream}. After that the program enters a loop to repeatedly read
the input of the connection. In \textcolor{orange}{line 5} a mutable \rust{String} named \rust{buf} is created to save
...
return a simple \rust{Result} but rather a \rust{Future} which wraps around the returned value. This means we can use
an \rust{await} point to tell the scheduler it can stop running the current program until new data has arrived.
...
write instance of the stream using a system call. Other than the \rust{shutdown} method of \rust{std::io::TcpStream},
...
\rust{async} blocks do not have an explicitly specified return type, the compiler sometimes fails to infer the error
...
\textcolor{teal}{Figure \ref{async-tcp-client-handeling}} into a function called \rust{handle_client} to be able to
call it in \textcolor{teal}{Figure \ref{async-tcp-listener}}. Next we are going to open the TCP streams and call the
\rust{handle_client} with the stream as an argument. It was already mentioned before that having TCP connections to
...
the listener ourselves. This method again returns a tuple including the \rust{TcpStream} and the address of the new
...
to read from the command line directly but rather wrap it using a \rust{tokio::io::BufReader} instance to be able to
...
\rust{String} called \rust{buf} is created to save the input of the user. Then the \rust{read_line} method with
...
the scheduler continues to execute the task, the return value of \rust{read_line} is matched.
...
same way. The read input is converted to bytes using \rust{as_bytes} and saved in the variable \rust{bytes}. After that
...
similar to the \rust{UdpSocket} of the standard library used in \textcolor{teal}{Figure \ref{udp-receiver}} and
...
seperated from each other as they are in the synchronous UDP example. Otherwise the struct \rust{std::sync::Arc} could
be used to share the ownership of the \rust{UdpSocket} to have a sending and receiving task in the same client. This is
possible, because both the \rust{send_to} and \rust{recv_from} methods of the socket do not require a mutable reference
